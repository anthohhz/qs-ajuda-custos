...
# Estes dois módulos contêm dados operacionais derivados de planilhas internas e são
# deliberadamente ignorados pelo Git. Em uma instalação nova, o QS deve iniciar sem
# esses arquivos; em uma máquina autorizada, se existirem localmente, eles continuam
# sendo usados para popular a base inicial.
try:
...
try:
    from .operational_seed_data import (
        EMPLOYEE_SNAPSHOT, BASE_TARIFFS, HOLIDAY_SNAPSHOT, HISTORICAL_OCCURRENCES,
    )
except ImportError:
    EMPLOYEE_SNAPSHOT = []
    BASE_TARIFFS = []
...
def ensure_periods(db, year: int, month: int, cutoff: int):
    """Garante as quinzenas QS e preserva períodos históricos fechados."""
...
def _seed_employees(db):
    ...
    for row in EMPLOYEE_SNAPSHOT:
        db.add(Employee(
            **row,
            source_system="Puzzle snapshot",
            active=True,
            in_scope=(row.get("benefit_group") != "ADMIN_OUTROS"),
        ))
    db.flush()
...
def _seed_tariffs(db):
    ...
    for row in BASE_TARIFFS:
        ...
        db.add(BaseTariff(
            ...
            city=row["city"],
            city_norm=canonical_city(row["city"]),
            uf=row["uf"],
            modal=row.get("modal"),
...
def _seed_holidays(db):
    ...
    for row in HOLIDAY_SNAPSHOT:
        ...
        db.add(Holiday(
            ...
            region=row["region"],
            region_norm=canonical_city(row["region"]),
            uf=row["uf"],
            reason=row.get("reason") or "Feriado",
...
def _seed_holiday_rules_2026(db):
    ...
    for date_text, name in FEDERAL_OPTIONAL_DAYS_2026:
        ...
        if not db.query(HolidayRule).filter(HolidayRule.source_key == key).first():
            db.add(HolidayRule(
                ...
                name=name,
                scope_type="NATIONAL",
                category="PONTO_FACULTATIVO",
                affects_calculation=False,
                source="OFICIAL_2026",
                source_ref=FEDERAL_SOURCE,
                notes="Visível no calendário para conferência; não altera o cálculo por padrão.",
                active=True,
            ))
    ...
    for old in db.query(Holiday).all():
        ...
        db.add(HolidayRule(
            source_key=key,
            date=old.date,
            name=old.reason or "Feriado municipal",
            scope_type="MUNICIPAL",
            uf=old.uf,
            city=old.region,
            city_norm=canonical_city(old.region),
            category="FERIADO",
            affects_calculation=(old.status == "OK"),
            source="MIGRACAO_EXCEL",
            source_ref=f"Calendário histórico migrado · registro {old.legacy_id}",
            active=True,
        ))
    db.flush()
...
def _sync_operational_cities(db):
    """Cria catálogo padronizado a partir das cidades já usadas pela operação."""
    candidates = []
    ...
    for holiday in db.query(HolidayRule).filter(
        HolidayRule.scope_type == "MUNICIPAL",
        HolidayRule.city != None,
        HolidayRule.uf != None,
    ).all():
    ...
    for uf_raw, city_raw, source in candidates:
        ...
        if key in seen:
            continue
        seen.add(key)
        if not db.query(City).filter(City.uf == uf, City.name_norm == name_norm).first():
...
def _seed_policies(db):
    ...
    policies = [
        {
            "policy_key": "PROMOTOR_44H",
            "label": "Promotor · 44h",
            "workload_key": "44H",
            "weekdays": "0,1,2,3,4,5",
            "vt_per_day": 2.0,
            "approved": False,
            "notes": "Legado V0.9.2 mantido somente para compatibilidade durante a migração do motor.",
        },
        {
            "policy_key": "PROMOTOR_40H",
            "label": "Promotor · 40h",
            "workload_key": "40H",
            "weekdays": "0,1,2,3,4",
            "vt_per_day": 2.0,
        ...
        {
            "policy_key": "PROMOTOR_MEI_6H",
            "label": "Promotor MEI · 6h",
            "workload_key": "6H_MEI",
            "weekdays": "0,1,2,3,4,5",
            "vt_per_day": 2.0,
            "approved": False,
...
def _name_variants(name: str):
    ...
    return {value for value in (normalized, stripped) if value}
...
def _seed_occurrences(db):
    ...
    for employee in employees:
        for key in _name_variants(employee.name):
    ...
    for row in HISTORICAL_OCCURRENCES:
        ...
        unique = {employee.id: employee for employee in matches}
        ...
        db.add(Occurrence(
            source_key=f"MIGRACAO:{row['legacy_id']}",
            employee_id=employee.id if employee else None,
            employee_name_text=row.get("employee_name"),
            kind=row.get("kind") or "OUTRO",
            start_date=start_date,
            end_date=end_date,
            impact_mode=row.get("impact_mode") or "INFO",
...
def _ensure_sqlite_migrations():
    """Migra banco persistente de versões anteriores sem apagar dados."""
